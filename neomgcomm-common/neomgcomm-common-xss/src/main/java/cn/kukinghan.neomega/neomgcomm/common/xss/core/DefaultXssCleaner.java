...
package cn.kukinghan.neomega.neomgcomm.common.xss.core;

import cn.hutool.core.util.CharsetUtil;
import cn.kukinghan.neomega.neomgcomm.common.xss.config.NeomgcommXssProperties;
import cn.kukinghan.neomega.neomgcomm.common.xss.utils.XssUtil;
...
public class DefaultXssCleaner implements XssCleaner {
	...
	private final NeomgcommXssProperties properties;

	public DefaultXssCleaner(NeomgcommXssProperties properties) {
		this.properties = properties;
	}

	private static Document.OutputSettings getOutputSettings(NeomgcommXssProperties properties) {
	...
	public String clean(String bodyHtml, XssType type) {
		...
		NeomgcommXssProperties.Mode mode = properties.getMode();
		if (NeomgcommXssProperties.Mode.escape == mode) {
			// html 转义
			return HtmlUtils.htmlEscape(bodyHtml, CharsetUtil.UTF_8);
		}
		else if (NeomgcommXssProperties.Mode.validate == mode) {
			// 校验
